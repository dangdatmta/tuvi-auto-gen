...
- The content pool is sequential: it uses all Sun Tzu lessons first, then continues into Confucius/Laozi lessons from `automation/extra-lessons.json`, and loops only after the whole pool is exhausted. The current pool has 18 lessons, so at 8 scheduled runs per day the sequence repeats after 18 videos, or 2.25 days. Set `VIDEO_SERIES_START_DATE` if you want to reset where the sequence begins.
