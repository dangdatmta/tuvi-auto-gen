...
- The content pool is sequential: it uses all Sun Tzu lessons first, then continues into Confucius/Laozi lessons from `automation/extra-lessons.json`, and loops only after the whole pool is exhausted. Set `VIDEO_SERIES_START_DATE` if you want to reset where the sequence begins.
...
- Writes platform-specific viral captions and tags into `post.json`; TikTok, YouTube Shorts, and Facebook Reels each use the caption tuned for that platform.
...
The workflow renders videos, uploads artifacts, and can publish directly when credentials are configured. Publishing is guarded by `ENABLE_SOCIAL_PUBLISH` and `PUBLISH_DRY_RUN`.
...
- TikTok: `TIKTOK_ACCESS_TOKEN` with Content Posting API access and `video.publish` scope.
